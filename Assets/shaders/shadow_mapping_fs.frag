...
uniform int shadowCalculationMethod;
...
float ShadowCalculationNoPCF(vec3 fragPos, vec3 lightPos)
{
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    float shadow;
    if (shadowCalculationMethod == 0) {
        shadow = ShadowCalculation(FragPosLightSpace, normal, lightDir, light.position, fragPos);
    } else if (shadowCalculationMethod == 1) {
        shadow = ShadowCalculationNoPCF(fragPos, light.position);
    } else {
        shadow = 0.0;
    }
